...
const calculateExercises = (dailyHours: Array<number>): ExercisesResult => {
  const target = dailyHours.shift()
  const trainingDays = dailyHours.length - count(dailyHours, 0)
  const rate = trainingDays / dailyHours.length
...
const count = (arr: Array<number>, num: number) => {
  let result = 0
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === num) {
      result++
